...
OSCL_EXPORT_REF bool AndroidSurfaceOutputFB::initCheck()
{
    ...
    if (iVideoSubFormat == PVMF_MIME_YUV420_SEMIPLANAR_YVU) {
        ...
        ISurface::BufferHeap buffers(displayWidth, displayHeight, 
                frameWidth, frameHeight, HAL_PIXEL_FORMAT_YCrCb_420_SP, mHeapPmem);
    ...
    if (mHardwareCodec) {
        ...
        if (mHeapPmem == 0) {
            ...
            ISurface::BufferHeap buffers(iVideoDisplayWidth, iVideoDisplayHeight, 
                    ...
                    HAL_PIXEL_FORMAT_YCrCb_420_SP, mHeapPmem);
            mSurface->registerBuffers(buffers);
        }
...
void AndroidSurfaceOutputFB::closeFrameBuf()
{
    ...
    LOGV("free mHeapPmem");
    /* Motorola - Temporary work around 
       Ensure that all references to mHeapPmem are released
    */
    int attempts = 4; /* total sleep = 2seconds */

    /* Motorola w17724 - free mHeapPmem only if not null bug 34250 */ 
    if(mHeapPmem != 0)
    { 
    if(mHeapPmem->getStrongCount() >= 2)
    {
        while(attempts)
        {
            usleep(250000);
            if(mHeapPmem->getStrongCount() == 1)
            {
                 mHeapPmem.clear();
                 break;
            }
...
extern "C" AndroidSurfaceOutput* __attribute__ ((visibility("default"))) createVideoMio()
{
